...
主动模式通过管理员专用 API，将账号选择限制在明确绑定到目标出口节点的 Grok Build 账号，并发送固定流式 Prompt。被动模式读取新的成功流式审计，按面板同口径 `输出 Token / (总耗时 - 首字耗时)` 计算速度；输出 Token 故意包含 Reasoning Token。
...
连续主动探测错误达到阈值后才隔离，避免单次瞬时失败误杀。后端把账号选择失败标记为独立的 `egressQualityProbeNoAccount`；守护遇到该错误只延后复测，不累计代理错误、不换 IP，也不会恢复未经验证的节点。
